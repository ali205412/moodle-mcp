...
- **Source of Truth**: Moodle-dependent decisions must be verified against local upstream Moodle source in `tmp/moodle` — do not rely on memory or docs alone when source is available
...
| Verify Moodle-dependent implementation choices against local upstream source | This project spans auth, sessions, externals, routing, wrappers, and version compatibility where guesswork is too risky | — Pending |
